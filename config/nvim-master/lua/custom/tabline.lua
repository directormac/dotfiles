...
---@param str string
local len = function(str) return api.nvim_strwidth(str) end
...
local truncate_by_display_width = function(str, max_width, from_end)
  ...
  if fn.strdisplaywidth(str) <= max_width then return str end
...
local calculate_unambiguous_paths = function(paths)
  ...
  while #ambiguous_paths > 0 and iteration < max_iterations do
    ...
    for _, path in ipairs(ambiguous_paths) do
      ...
      if not alias_groups[alias] then alias_groups[alias] = {} end
    ...
    for alias, group_paths in pairs(alias_groups) do
      if #group_paths == 1 then
        ...
        for _, path in ipairs(group_paths) do
          ...
          if path_to_level[path] < #path_parts[path] then path_to_level[path] = path_to_level[path] + 1 end
        end
      end
    end
...
local cleanup_bufs = function(base_bufs)
  ...
  for _, base in ipairs(processed_bufs) do
    if path_aliases[base.name] then base.name = path_aliases[base.name] end

    if vim.wo[base.winnr].diff then base.name = ' ' .. base.name end

    if vim.bo[base.bufnr].modified then base.name = base.name .. ' ●' end
...
M.render = function()
  ...
  if len(cur_tab_data.name) + padding >= cols then
    ...
    if not is_init then line = line .. hl_more end

    line = line .. '%#' .. mode_hl .. '#' .. trunc .. CUT

    if not is_last then line = line .. hl_more end
    ...
    for _, buf in ipairs(processed_bufs) do
      ...
      if buf.page == cur_idx then cur_mid = (2 * content_len + buf_len) / 2 end
